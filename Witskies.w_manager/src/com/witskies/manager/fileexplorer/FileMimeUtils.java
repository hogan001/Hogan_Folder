 ...
 * Extensions)多用途互联网邮件扩展类型。是设定某种扩展名的文件用一种应用程序来打开的方式类�? * ，当该扩展名文件被访问的时�?，浏览器会自动使用指定应用程序来打开。多用于指定�?��客户端自定义的文件名，以及一些媒体文件打�?��式�? for dealing
 * with MIME types. Used to implement java.net.URLConnection and
