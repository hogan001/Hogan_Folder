...
public class FileOperationHelper {
    ...
    public boolean canMove(String path) {
        for (FileInfo f : mCurFileNameList) {
            ...
            if (FileUtil.containsPath(f.filePath, path))
                return false;
        }
    ...
    public boolean Rename(FileInfo f, String newName) {
        ...
        String newPath = FileUtil.makePath(FileUtil.getPathFromFilepath(f.filePath), newName);
        ...
        if (file.isDirectory()) {
            ...
            for (File child : file.listFiles(mFilter)) {
                if (!child.isHidden() && FileUtil.isNormalFile(child.getAbsolutePath())) {
                    CopyFile(FileUtil.GetFileInfo(child, mFilter, FileSettings.instance().getShowDotAndHiddenFiles()), destPath);
                }
            }
        } else {
