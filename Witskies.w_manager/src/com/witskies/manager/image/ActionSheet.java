...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	private void createItems() {
		...
		if (titles != null) {
			for (int i = 0; i < titles.length; i++) {
				otherButton = new Button(getActivity());
				otherButton.setId(CANCEL_BUTTON_ID + i + 1);
				otherButton.setOnClickListener(this);
				otherButton.setBackgroundDrawable(getOtherButtonBg(titles, i));
				otherButton.setText(titles[i]);
				otherButton.setTextColor(mAttrs.otherButtonTextColor);
				otherButton.setTextSize(TypedValue.COMPLEX_UNIT_PX,
				...
				if (i > 0) {
					...
					mPanel.addView(otherButton, params);
				} else {
					mPanel.addView(otherButton);
				}
			}
		}
		cancleButton = new Button(getActivity());
		cancleButton.getPaint().setFakeBoldText(true);
		cancleButton.setTextSize(TypedValue.COMPLEX_UNIT_PX,
				mAttrs.actionSheetTextSize);
		cancleButton.setId(CANCEL_BUTTON_ID);
		cancleButton.setBackgroundDrawable(mAttrs.cancelButtonBackground);
		cancleButton.setText(getCancelButtonTitle());
		cancleButton.setTextColor(mAttrs.cancelButtonTextColor);
		cancleButton.setOnClickListener(this);
		LinearLayout.LayoutParams params = createButtonLayoutParams();
		params.topMargin = mAttrs.cancelButtonMarginTop;
		mPanel.addView(cancleButton, params);
	...
	public void onClick(View v) {
		...
		if (v.getId() == 102) {
			if (otherButton.getText().equals("ȫ��ѡ")) {
				otherButton.setText("ȫѡ");
			} else {
				otherButton.setText("ȫ��ѡ");
